...
- Skill imports reject collisions unless `--force` is supplied. Rename preserves locally edited provider copies. Link/unlink never deletes the original source directory.
...
- Plugin status records ACM-managed installs, including compatible legacy `installedFor` metadata. Changes made independently in a provider can require reconciling or reinstalling through ACM. A running provider session may need restarting.
- Multiple provider mutations are sequential, with successful earlier targets retained on a later failure. Repeat for the failed target after addressing its error.
...
Subprocess tests use temporary homes and catalogs. Provider commands are replaced by test executables, so native state changes are observable without editing a developer's actual settings. Live registry availability and all provider versions are not established by those fixtures.
