...
The current shared Catalog is especially sensitive. `~/.acm/plugins`, `plugins-metadata.toml`, and related paths can be symbolic links into the separate `the linked Catalog` Git repository. That repository can already contain unrelated changes, and its `PUBLIC.txt` controls separate publication behavior. Normal plugin workflows must not mutate it.
...
Tests always use a temporary HOME and temporary Catalog links. They must never scan or mutate the developer's actual `~/.acm` or the linked Catalog trees.
