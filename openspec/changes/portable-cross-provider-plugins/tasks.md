...
- [ ] 1.3 Add filesystem guards so plugin tests and fake adapters fail if they attempt to access the real HOME, `~/.acm`, or `~/.kanade`.
...
- [ ] 10.6 Review the final diff against all four capability specs, verify no real `~/.kanade/catalogs` or existing `.agents/` content changed, and leave an implementation handoff noting completed tasks and deferred non-goals.
