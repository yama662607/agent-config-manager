# workspace-mcp-management Specification
...
Run `acsync mcp <name>` for details, `acsync mcp add` to add new servers.
```
...
- **WHEN** a user runs `acsync mcp` or the equivalent explicit list command inside a managed project
...
- **WHEN** a user runs `acsync mcp --verbose`
...
- **WHEN** a user runs `acsync mcp init` inside a managed project
...
- **WHEN** a user runs `acsync mcp add @modelcontextprotocol/server-github --targets codex,claude-code`
...
- **WHEN** a user adds, removes, enables, or disables an MCP through `acsync`
