...
fn project_for_claude(catalog_plugin_dir: &Path, target_plugin_dir: &Path) -> anyhow::Result<()> {
    ...
    // Claude can link directly if it has .claude-plugin or plugin.json
    #[cfg(unix)]
...
/// Create a target-native projection for Google Antigravity
fn project_for_antigravity(catalog_plugin_dir: &Path, target_plugin_dir: &Path) -> anyhow::Result<()> {
    ...
    // Ensure Antigravity compatibility: if plugin.json doesn't exist in root, generate it
    let root_plugin_json = target_plugin_dir.join("plugin.json");
    let claude_plugin_json = target_plugin_dir.join(".claude-plugin").join("plugin.json");
    if !root_plugin_json.exists() && claude_plugin_json.exists() {
        if let Ok(content) = fs::read_to_string(&claude_plugin_json) {
            let _ = fs::write(&root_plugin_json, content);
        }
    }

    // Ensure Antigravity mcp_config.json compatibility
    let dot_mcp = target_plugin_dir.join(".mcp.json");
    let mcp_config = target_plugin_dir.join("mcp_config.json");
    if !mcp_config.exists() && dot_mcp.exists() {
        if let Ok(content) = fs::read_to_string(&dot_mcp) {
            let _ = fs::write(&mcp_config, content);
        }
    }
            ...
            TargetName::Codex => {
                ...
                for skill_name in &info.skills {
                    ...
                    if dst_skill.exists() || fs::symlink_metadata(&dst_skill).is_ok() {
                        let _ = fs::remove_file(&dst_skill).or_else(|_| fs::remove_dir_all(&dst_skill));
                    }
                    #[cfg(unix)]
                ...
                for (name, recipe) in &info.mcp_servers {
                    let _ = codex::add_mcp_to_config(&codex_config_path, name, recipe);
                }
            }
            TargetName::Grok => {
                ...
                if plugin_skills_dir.exists() {
                    let _ = grok::register_skill_path(&grok_config_path, &plugin_skills_dir.to_string_lossy());
                }

                // 2. Inject MCP servers into Grok config.toml
                for (name, recipe) in &info.mcp_servers {
                    let _ = grok::add_mcp_to_config(&grok_config_path, name, recipe);
                }
            }
        }
    }
...
pub fn plugin_add_to_catalog<P: AsRef<Path>>(source_path: P, custom_id: Option<&str>) -> anyhow::Result<String> {
    ...
    let parsed = parse_plugin_dir(src)?;
    ...
    for &target in targets {
        let state = match target {
            ...
            TargetName::Codex => {
                ...
                let any_skill_present = info.skills.iter().any(|s| skills_dir.join(s).exists());
                ...
                let any_mcp_present = info.mcp_servers.keys().any(|k| mcps.contains_key(k));
