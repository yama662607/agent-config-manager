...
pub fn validate_skill_directory<P: AsRef<Path>>(skill_dir: P) -> anyhow::Result<Vec<ValidationIssue>> {
...
pub fn validate_skill_content(content: &str, expected_dir_name: &str) -> Vec<ValidationIssue> {
    ...
    let caps = match re.captures(content) {
        ...
        None => {
            issues.push(ValidationIssue {
                severity: IssueSeverity::Error,
                message: "Missing YAML Frontmatter (must start with '---' and end with '---')".to_string(),
            });
            return issues;
        }
    };
