...
pub fn load_skills_metadata() -> anyhow::Result<SkillsMetadataFile> {
    ...
    let data: SkillsMetadataFile =
        toml::from_str(&content).context("Invalid skills-metadata.toml")?;
    Ok(data)
}
...
pub fn save_skills_metadata(data: &SkillsMetadataFile) -> anyhow::Result<()> {
    ...
    crate::storage::update_value(&path, |value| {
...
pub fn load_mcps_metadata() -> anyhow::Result<McpsMetadataFile> {
    ...
    let data: McpsMetadataFile = toml::from_str(&content).context("Invalid mcps-metadata.toml")?;
    Ok(data)
}
...
pub fn update_skill_metadata<T>(
    id: &str,
    update: impl FnOnce(&mut serde_json::Value) -> anyhow::Result<T>,
) -> anyhow::Result<T> {
    crate::storage::update_value(&get_skills_metadata_path(), |value| {
        let entry = crate::storage::object_at(value, "skills")?
            .entry(id)
