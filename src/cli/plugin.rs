...
pub fn run(ctx: &ContextOptions, args: PluginArgs) -> anyhow::Result<()> {
    ...
    match args.command.unwrap_or(PluginSubcommands::List) {
        PluginSubcommands::List => ctx.output(get_plugin_workspace_status(&ctx.plugin_root()?, &ctx.targets)?.plugins),
        ...
        PluginSubcommands::Install { id } => {
            ...
            ctx.output(plugin_install(&ctx.plugin_root()?, &id, &ctx.targets)?)
        }
        PluginSubcommands::Remove { id, keep_skills } => {
            ...
            if keep_skills {
                ...
                for skill in plugin.skills {
                    ...
                    crate::core::skill::skill_add(&root, &skill, &ctx.targets, None)?;
                }
            }
        ...
        PluginSubcommands::Convert { id, all, dry_run, assemble_only } => {
            ...
            if dry_run { return ctx.output(json!({"dryRun": true, "plugins": ids, "marketplace": get_catalog_dir().join("marketplace")})); }
            let marketplace = build_marketplace(&ids)?;
            if !assemble_only && !ctx.catalog { register_marketplace(&ctx.plugin_root()?, &ctx.targets, &marketplace)?; }
            ctx.output(json!({"plugins": ids, "marketplace": marketplace}))
        }
        PluginSubcommands::Update { ids, all, dry_run, force } => {
            let ids = if all || ids.is_empty() { catalog_plugin_ids()? } else { ids };
            let root = ctx.plugin_root()?;
            ctx.output(ids.iter().map(|id| plugin_update_options(&root, id, if ctx.catalog { &[] } else { &ctx.targets }, force, dry_run)).collect::<anyhow::Result<Vec<_>>>()?)
        }
