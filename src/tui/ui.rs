...
fn render_skills_tab(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let items: Vec<ListItem> = filtered_skills
        ...
        .map(|(idx, skill)| {
            ...
            let icon_color = if skill.enabled {
                Color::Green
            } else {
                Color::DarkGray
            };
            ...
            for target in &app.targets {
                let badge = match skill.placement.get(target) {
                    Some(SkillPlacementState::Linked) => {
                        (format!("{}:L", target.short_code()), Color::Green)
                    }
                    Some(SkillPlacementState::CopyCurrent) => {
                        (format!("{}:C", target.short_code()), Color::Cyan)
                    }
                    Some(SkillPlacementState::CopyStale) => {
                        (format!("{}:!", target.short_code()), Color::LightRed)
                    }
                    Some(SkillPlacementState::BrokenLink) => {
                        (format!("{}:X", target.short_code()), Color::Red)
                    }
                    Some(SkillPlacementState::Registered) => {
                        (format!("{}:R", target.short_code()), Color::Blue)
                    }
    ...
    let selected_skill = filtered_skills
        .get(app.selected_skill_index)
        .map(|s| (*s).clone());
    ...
    if let Some(selected_skill) = selected_skill {
        ...
        lines.push(Line::from(Span::styled(
            format!(
                "SKILL.md Preview (Scroll: J/K or PgDn/PgUp) [Scroll: {}]",
                app.preview_scroll
            ),
        ...
        .map(|(idx, plugin)| {
            ...
            for target in &app.targets {
                let badge = match plugin.placement.get(target) {
                    Some(PluginPlacementState::NativeLinked) => {
                        (format!("{}:L", target.short_code()), Color::Green)
                    }
                    Some(PluginPlacementState::ConvertedLinked) => {
                        (format!("{}:C", target.short_code()), Color::Cyan)
                    }
    ...
    if let Some(selected_plugin) = selected_plugin {
        ...
        lines.push(Line::from(Span::styled(
            format!(
                ...
                selected_plugin.mcp_servers.len()
            ),
            Style::default().fg(Color::Cyan),
        )));
        ...
        let detail_widget = Paragraph::new(lines)
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .title(" Plugin Details & Components "),
            )
            .wrap(Wrap { trim: false });
        f.render_widget(detail_widget, panes[1]);
    }
...
fn render_mcp_tab(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let selected_mcp = filtered_mcps
        .get(app.selected_mcp_index)
        .map(|m| (*m).clone());
    ...
    if let Some(selected_mcp) = selected_mcp {
        ...
        let detail_widget = Paragraph::new(lines)
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .title(" Server Recipe & Target Status "),
            )
            .wrap(Wrap { trim: false });
        f.render_widget(detail_widget, panes[1]);
    }
...
fn render_doctor_tab(f: &mut Frame, app: &App, area: Rect) {
    if let Some(report) = &app.doctor_report {
        ...
        let empty = Paragraph::new("No diagnostic report available.").block(
            Block::default()
                .borders(Borders::ALL)
                .title(" Diagnostics "),
        );
